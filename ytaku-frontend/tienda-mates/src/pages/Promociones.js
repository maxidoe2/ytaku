  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        ...
        const enOfertaConStock = data.filter(
          (prod) =>
            prod.oferta === true &&
        ...
        const tiposUnicos = [
          ...new Set(enOfertaConStock.map((prod) => prod.tipo))
        ].sort();

        const maxPrecio = Math.max(...enOfertaConStock.map(p =>
          Math.max(...p.variantes.map(v => parseFloat(v.precio) || 0))
        ));

        setPrecioRange([0, maxPrecio]);
        setProductos(enOfertaConStock);
